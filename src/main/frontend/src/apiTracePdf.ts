...
import { inScopeCount, NO_ROUTE, releaseScopeGroups, scopeGroups } from './catalog';
...
export async function exportApiTracePdf(mods: ModuleCat[], app?: string, version?: string, country?: string) {
  ...
  r.dataTable(
    ...
    rows.map((x) => [x.name + (x.error ? '  (failed)' : ''), x.error ? '—' : (x.unversioned ? 'Base' : ver), x.apis, x.routes, x.backends]),
  ...
  r.legend('How to read this report', [
    ...
    'Under each module, APIs are split into Release (on this release’s route) and Base (no route at this release — resolved to their base route).',
    'The module’s headline count is the Release APIs; Base APIs are listed for reference and not counted as in-scope.',
  ...
  for (const m of mods) {
    ...
    const cat = m.cat;
    if (!cat) continue;
    // Group by resolution: release-versioned APIs vs base-route APIs (regardless of the release picked).
    const traces = scopeGroups(cat).flatMap((g) => g.traces);
    const release = traces.filter((t) => !t.baseFallback);
    const base = traces.filter((t) => t.baseFallback);
    const headline = inScopeCount(cat);
    r.section('Module — ' + m.name, headline, cat.unversioned ? PAL.amber : PAL.blue,
      cat.unversioned ? 'Unversioned module — every API resolves to its base route.'
        : base.length ? `${release.length} API(s) on release ${ver}, plus ${base.length} on their base route (no ${ver} version).` : '');
    ...
    if (noRouteGroup && noRouteGroup.traces.length) {
      r.y += 6;
      r.para(`No route found (${noRouteGroup.traces.length}): ` + noRouteGroup.traces.map((t) => t.operationName || t.api || '(unknown)').join(', '),
        M, CONTENT_W, 'normal', 9, PAL.muted, 12);
    }
...
function apiRow(r: ReportDoc, t: TraceResponse, svc: Record<string, string>) {
  ...
  // A base-resolved API shows Base (amber); a versioned one shows Release <ver>.
  const pillLabel = t.baseFallback ? 'Base' : (t.resolvedVersion ? 'Release ' + t.resolvedVersion : '');
  if (pillLabel) {
  ...
  r.y += 16;
  // Flow is intentionally omitted — this is a shared release report; the entry route + backends
  // are the level of detail other teams need.
...
/** Compact one-line row for a base-route API (path -> route), under the amber "Base" heading. */
function baseRow(r: ReportDoc, t: TraceResponse) {
  r.ensure(14);
  const path = t.api || t.operationName || '(unknown)';
  const w = r.text(path, M + 4, 'bold', 10, PAL.ink);
  r.text('->  ' + (t.resolvedRoute || '-'), M + 4 + w + 8, 'normal', 9, PAL.muted);
  r.y += 13;
}
